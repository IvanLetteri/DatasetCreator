    ...
    def compute_features(self, packets_list):
        ...
        def compute_packet_TCP_payload_size(packets_list, count_packet_without_payload=False):
            ...
            for pkt in packets_list:
                if (pkt.haslayer("TCP")):
                    if (pkt[
                        "TCP"].payload == None):  # Il pacchetto e' TCP ma non ha payload. Probabilemente e' un three way
        ...
        row = [syn_avg, fin_avg, ack_avg, psh_avg, urg_avg, rst_avg, dns_pkt, tcp_pkt, udp_pkt, icmp_pkt, durationFlow, avgTimeFlow,
